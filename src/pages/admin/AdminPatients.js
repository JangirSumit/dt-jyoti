  ...
  return (
    ...
    <Paper sx={{ p: { xs: 1.5, md: 2 }, mb: 2 }}>
      ...
      {patients.length === 0 ? <Typography>No patients.</Typography> : (
        <Box>
          {patients.map((p) => (
            <Paper key={p.id} sx={{ p: { xs: 1.25, md: 1.5 }, my: 1, borderRadius: 2 }}>
              <Typography><b>{p.name}</b> — {p.contact}{p.email ? `, ${p.email}` : ''}</Typography>
              {p.notes && <Typography color="text.secondary" sx={{ wordBreak: 'break-word', overflowWrap: 'anywhere' }}>Notes: {p.notes}</Typography>}
            </Paper>
          ))}
        </Box>
      )}
